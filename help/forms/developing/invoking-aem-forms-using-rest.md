...
Om de dienst van Forms aan te halen (een proces wordt de dienst wanneer het proces) gebruikend REST wordt geactiveerd, vorm een REST eindpunt. (Zie &quot;Eindpunten beheren&quot; in [beleidsHelp](https://www.adobe.com/go/learn_aemforms_admin_63).)
...
* Verzamelingsobjecten zoals `java.util.List` en `java.util.Map`
   ...
   Als een dienst van Forms een `com.adobe.idp.Document` parameter vereist, kan de dienst slechts worden aangehaald gebruikend de methode van de POST van HTTP. Als de service één `com.adobe.idp.Document` parameter vereist, wordt de HTTP-aanvraaginstantie de inhoud van het invoerobject Document.
...
Sommige AEM Forms-diensten, zoals menselijke-centrische langlevende processen, hebben lange tijd nodig om te voltooien. Deze diensten kunnen asynchroon op een niet-blokkerende manier worden aangehaald. (Zie [Langdurige processen](/help/forms/developing/invoking-human-centric-long-lived.md#invoking-human-centric-long-lived-processes)aanroepen vanuit menselijk perspectief.)
...
In het volgende HTML-voorbeeld wordt een AEM Forms-proces aangeroepen `MyApplication/EncryptDocument` waarvoor een PDF-document vereist is. Zie AEM Forms [aanroepen met MTOM](/help/forms/developing/invoking-aem-forms-using-web.md#invoking-aem-forms-using-mtom)voor informatie over dit proces.
...
In het volgende HTML-voorbeeld wordt een AEM Forms-proces aangeroepen `SOAPEchoService` waarvoor een opsommingswaarde is vereist. De methode HTML Post wordt gebruikt.
...
U kunt een Forms-proces aanroepen vanuit Acrobat met behulp van een REST-aanvraag. U kunt bijvoorbeeld het proces *MyApplication/EncryptDocument* activeren. Als u een Forms-proces vanuit Acrobat wilt aanroepen, plaatst u een verzendknop in een XDP-bestand in Designer. (Zie [Help bij](https://www.adobe.com/go/learn_aemforms_designer_63)Designer.)
