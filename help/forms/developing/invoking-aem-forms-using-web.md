...
>[!NOTE]
>
>U wordt aangeraden een `BLOB` object niet groter dan 30 MB te laten vullen door de `setBinaryData` methode ervan aan te roepen. Anders bestaat de mogelijkheid dat een `OutOfMemory`* uitzondering optreedt.*
...
1. Gebruik JAX-WS en Apache Ant om Java-proxyklassen te genereren. Maak een Ant-constructiescript om deze taak uit te voeren. Het volgende script is een voorbeeldscript voor Ant-build met de naam build.xml:
   ...
   >[!NOTE]
   >
   >Vervang `EncryptionService`* door de servicenaam AEM Forms die u wilt gebruiken met Java-proxyklassen. Als u bijvoorbeeld Java-proxyklassen wilt maken voor de Rights Management-service, geeft u het volgende op:*
