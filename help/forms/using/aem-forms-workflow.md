...
description: Gebruik AEM Forms Workflow om revisie en goedkeuringen te automatiseren en snel samen te stellen, documentservices te starten (bijvoorbeeld voor het converteren van een PDF-document naar een andere indeling), te integreren met de Adobe Sign-handtekeningworkflow en nog veel meer.
...
* Een workflow kan ook Adobe-handtekeningen gebruiken voor digitale handtekeningen. Als u van plan bent om Adobe Sign in een werkstroom te gebruiken, [vorm Adobe Sign voor AEM Forms](/help/forms/using/adobe-sign-integration-adaptive-forms.md) alvorens het in een werkschema te gebruiken.

## Een workflowmodel maken {#create-a-workflow-model}

Een workflowmodel bestaat uit logica en stroom van een bedrijfsproces. Het bestaat uit een reeks stappen. Deze stappen zijn AEM componenten. U kunt workflowstappen uitbreiden met parameters en scripts om desgewenst meer functionaliteit en controle te bieden. AEM Forms bevat een aantal stappen naast AEM stappen uit het vak. Voor een gedetailleerde lijst van AEM en AEM Forms stappen, zie [AEM Referentie van de Stap van de Werkstroom](/help/sites-developing/workflows-step-ref.md) en [Forms-centric werkschema op OSGi - Step Referentie](/help/forms/using/aem-forms-workflow.md).
...
1. Selecteer **[!UICONTROL Create]** en **[!UICONTROL Create Model]**. Het dialoogvenster Workflowmodel toevoegen wordt weergegeven.
1. Voer **[!UICONTROL Title]** en **[!UICONTROL Name]** (optioneel) in. Bijvoorbeeld een hypotheekaanvraag. Tik op **[!UICONTROL Done]**.
...
1. Sleep de component OR Splitsen van de stapbrowser naar het workflowmodel. Met de indeling OR wordt een splitsing in de workflow gemaakt, waarna slechts één vertakking actief is. Met deze stap kunt u voorwaardelijke verwerkingspaden in uw workflow introduceren. U voegt workflowstappen naar wens toe aan elke vertakking.
   ...
   **Codefragment voor takken 1**
   ...
   Wanneer een gebruiker **[!UICONTROL Approve]** in AEM Inbox tikt, wordt Tak 1 geactiveerd.
   ...
   **Codefragment voor vertakking 2**
   ...
   Wanneer een gebruiker **[!UICONTROL Reject]** in AEM Inbox tikt, wordt Tak 2 geactiveerd.
...
1. Ga in de AEM auteur naar ![tools](assets/tools.png) > **[!UICONTROL Forms]** > **[!UICONTROL Manage Workflow Application]** en tikken **[!UICONTROL Create]**.
1. Geef in het venster Workflowtoepassing maken invoer op voor de volgende velden en tik **[!UICONTROL Create]**. Er wordt een nieuwe toepassing gemaakt en deze wordt weergegeven in het scherm Workflowtoepassingen.
...
De workflowtoepassing die u hebt gemaakt, is beschikbaar als een toepassing in Inbox. Gebruikers die lid zijn van een groep workflowgebruikers kunnen de toepassing die de bijbehorende workflow activeert, invullen en verzenden. Zie [Forms-toepassingen en -taken beheren in AEM Inbox](/help/forms/using/manage-applications-inbox.md) voor informatie over het gebruik van AEM Inbox om toepassingen te verzenden en taken te beheren.
...
U kunt de verzendacties van een adaptief formulier zo configureren dat een workflow wordt gestart bij het verzenden van het adaptieve formulier. Aangepaste formulieren bieden de verzendactie **[!UICONTROL Invoke an AEM Workflow]** om een workflow te starten bij het verzenden van een adaptief formulier. Zie [De handeling Verzenden configureren](/help/forms/using/configuring-submit-actions.md) voor gedetailleerde informatie over de verzendactie. Als u een adaptief formulier wilt verzenden via de AEM Forms-app, schakelt u Synchroniseren met AEM Forms App in de adaptieve formuliereigenschappen in.
...
1. Ga voor de AEM auteur naar ![tools](assets/tools.png) **[!UICONTROL Forms > Configure Watched Folder]**. Er wordt een lijst met al geconfigureerde gecontroleerde mappen weergegeven.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Selecteer de optie <span class="uicontrol">Workflow </span>. </td> 
  </tr> 
  <tr> 
...
1. Tik op **[!UICONTROL Advanced]**. Geef een waarde op voor het volgende veld en tik **[!UICONTROL Create]**. De gecontroleerde map is geconfigureerd om een workflow te starten. Wanneer nu een bestand in de invoermap van de Gecontroleerde map wordt geplaatst, wordt de opgegeven workflow geactiveerd.
   ...
   Het tabblad Geavanceerd bevat meer velden. De meeste van deze velden bevatten een standaardwaarde. Zie [Een gecontroleerde map maken of configureren](/help/forms/using/creating-configure-watched-folder.md) voor meer informatie over alle velden.

### Een interactieve communicatie of een brief {#letter} indienen

U kunt een Forms-centric werkschema op OSGi associëren en uitvoeren op voorlegging van een interactieve mededeling of een brief. In correspondentiebeheerworkflows worden gebruikt voor interactieve communicatie en brieven na verwerking. Bijvoorbeeld het e-mailen, afdrukken, faxen of archiveren van uiteindelijke brieven. Zie [Nabewerking van interactieve communicatie en letters](/help/forms/using/submit-letter-topostprocess.md) voor gedetailleerde stappen.
...
Door het minimaliseren van het aantal workflowexemplaren worden de prestaties van de workflow-engine verbeterd, zodat u regelmatig voltooide of actieve workflowexemplaren uit de repository kunt verwijderen. Zie [Regelmatig leegmaken van workflowinstanties](/help/sites-administering/workflows-administering.md#regular-purging-of-workflow-instances) voor gedetailleerde informatie.
