...
description: De stappen u moet volgen om met AEM Forms over het Melden van het Proces van JEE te beginnen
seo-description: De stappen u moet volgen om met AEM Forms over het Melden van het Proces van JEE te beginnen
...
De diensten van de Rapportering van het Proces publiceren gegevens van het gegevensbestand van AEM Forms aan de plaats van de Rapportering van het Proces op een geplande basis.

Deze bewerking kan bronintensief zijn en de prestaties van de AEM Forms-servers beïnvloeden. U wordt geadviseerd om dit buiten uw server van AEM Forms bezige tijdgroeven te plannen.
...
>[!NOTE]
>
>Als u uw implementatie van AEM Forms op een cluster in werking stelt, voer de volgende stappen op elke knoop van de cluster uit.

#### JBoss-toepassingsserver {#jboss-application-server}

1. Stop de serverinstantie van AEM Forms.
...
1. Het JVM-argument toevoegen `-Dreporting.publisher.cron = <expression>.`
   ...
   Voorbeeld: Met de volgende uitsnijdexpressie worden bij Process Reporting om de vijf uur gegevens van AEM Forms naar de Process Reporting-opslagplaats gepubliceerd:

   * `-Dreporting.publisher.cron = 0_0_0/5_*_*_?`

1. Sla het `run.conf.bat` bestand op en sluit het.

1. Start de serverinstantie AEM Forms opnieuw.
...
U kunt de console van het Beleid gebruiken om de wortel (**gebrek**) te vormen: `/content/reporting/pm`) locatie en submap (**standaard**: `/yyyy/mm/dd/hh/mi/ss`) hiërarchieopmaak waarin de procesgegevens worden opgeslagen.
...
De dienst ProcessDataPublisher voert procesgegevens van het gegevensbestand van AEM Forms in en publiceert de gegevens aan de dienst ProcessDataStorageProvider voor opslag.
...
Telkens als de dienst ProcessDataPublisher loopt, verdeelt de dienst eerst de tijd sinds de laatste looppas van de dienst door het Interval van de Partij. De dienst verwerkt dan elk interval van AEM Forms gegevens afzonderlijk.
...
De omgeving van AEM Forms bevat gegevens uit de tijd dat de omgeving werd ingesteld.

Door gebrek, voert de dienst ProcessDataPublisher alle gegevens uit het gegevensbestand van AEM Forms in.
...
Nadat u de Rapportering van het Proces van de opstelling hebt, kunt u beginnen met Proces Meldend bij de volgende plaats in uw AEM Forms installatie te werken:
...
>[!NOTE]
>
>Om aan login aan het Proces Meldend gebruikersinterface, hebt u de volgende toestemming van AEM Forms nodig:
>
>`PERM_PROCESS_REPORTING_USER`

![vastleggen](assets/capture.png)

Als u zich aanmeldt bij Process Reporting, wordt het **[!UICONTROL Home]** scherm weergegeven.
...
**Tijd laatste update:** De procesgegevens worden op een geplande basis gepubliceerd vanuit de gegevensbank AEM Forms naar de gegevensopslagplaats voor procesrapportage.
