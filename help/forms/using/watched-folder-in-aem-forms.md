...
* Creeer een omslag op het dossiersysteem alvorens een Gecontroleerd eindpunt van de Omslag te vormen, en dan de volledige weg in het folderPath bezit te verstrekken. Zie [Gecontroleerde mapeigenschappen](/help/forms/using/watched-folder-in-aem-forms.md#main-pars-header-1)voor meer informatie over de eigenschap folderPath.
...
1. Voeg de volgende eigenschappen toe aan het knooppunt:
   ...
   * `inputProcessorId`
   * `outputFilePattern`
   Zie [Gecontroleerde mapeigenschappen](/help/forms/using/watched-folder-in-aem-forms.md#main-pars-header-1)voor een volledige lijst met ondersteunde eigenschappen.
...
* **excludeFilePattern (String)**: een puntkomma **;** Gescheiden lijst met patronen die een gecontroleerde map gebruikt om te bepalen welke bestanden en mappen worden gescand en opgehaald. Bestanden of mappen met dit patroon worden niet gescand voor verwerking. Deze instelling is handig wanneer de invoer een map met meerdere bestanden is. De inhoud van de map kan worden gekopieerd naar een map met een naam die wordt opgehaald door de gecontroleerde map. Hierdoor wordt voorkomen dat de gecontroleerde map een map opneemt die moet worden verwerkt voordat de map volledig is gekopieerd naar de invoermap. De standaardwaarde is null.

   U kunt [bestandspatronen](/help/forms/using/watched-folder-in-aem-forms.md#p-file-and-folder-patterns-p) gebruiken om uit te sluiten:

   * Bestanden met specifieke bestandsnaamextensies; bijvoorbeeld &amp;ast;.dat, &amp;ast;.xml, .pdf, &amp;ast;.&amp;ast;
   * Bestanden met specifieke namen; bijvoorbeeld data&amp;ast; bestanden en mappen met de naam data1, data2 enzovoort uitsluiten.
...
Zie [Bestandspatronen](/help/forms/using/watched-folder-in-aem-forms.md#p-file-and-folder-patterns-p)voor meer informatie over bestandspatronen.

* **includeFilePattern (String)**: een puntkomma **;** gescheiden lijst met patronen die de gecontroleerde map gebruikt om te bepalen welke mappen en bestanden moeten worden gescand en opgepakt. Als IncludeFilePattern bijvoorbeeld input&amp;ast; is, alle dossiers en omslagen die input&amp;amp aanpassen;ast; worden opgepakt. Dit omvat bestanden en mappen met de naam input1, input2, enzovoort. De standaardwaarde is &amp;ast; en geeft alle bestanden en mappen aan. U kunt bestandspatronen gebruiken om het volgende op te nemen:
...
Zie [Bestandspatronen voor meer informatie over bestandspatronen](/help/forms/using/watched-folder-in-aem-forms.md#p-file-and-folder-patterns-p)
...
**Opmerking:** *If het gegevenstype van het bezit is Double, dan specificeer een decimaal punt in de waarde van dergelijke eigenschappen. Voor alle eigenschappen, waarbij het gegevenstype Dubbel is en er geen decimaalteken is opgegeven in de waarde, wordt het type omgezet in Lang. *
...
1. Gebruik [AEM-bundelconsole](http://localhost:4502/system/console/bundles) om de bundel te installeren.
...
* Als de AEM Forms-server uitvalt voordat Gecontroleerde map de taken activeert, kan de beheerder de bestanden uit de werkgebiedmap verplaatsen. Zie [Foutpunten en herstel](/help/forms/using/watched-folder-in-aem-forms.md#p-failure-points-and-recoveryfailure-points-and-recovery-p)voor meer informatie.
* Als de AEM Forms-server wordt uitgevoerd maar de gecontroleerde map niet wordt uitgevoerd wanneer de taakbeheerservice weer wordt aangeroepen, wat gebeurt wanneer services niet in de geordende volgorde worden gestart, kan de beheerder de bestanden uit de werkgebiedmap verplaatsen. Zie [Foutpunten en herstel](/help/forms/using/watched-folder-in-aem-forms.md#p-failure-points-and-recoveryfailure-points-and-recovery-p)voor meer informatie.
...
* Bestanden met specifieke bestandsextensies; bijvoorbeeld &amp;ast;.dat, &amp;ast;.xml, .pdf, &amp;ast;.&amp;ast;
...
1. [Een ECMAScript maken](/help/forms/using/watched-folder-in-aem-forms.md#p-create-an-ecmascript-p)
1. [Een workflow maken](/help/forms/using/watched-folder-in-aem-forms.md#p-create-a-workflow-p)
1. [De gecontroleerde map configureren](/help/forms/using/watched-folder-in-aem-forms.md#p-configure-the-watched-folder-p)
...
1. Voeg de volgende eigenschappen toe aan het knooppunt:
   ...
   * outputFilePattern (String): Patroon van het uitvoerbestand. U kunt een map of bestandspatroon opgeven. Als een mappatroon is opgegeven, hebben de uitvoerbestanden de namen die in de workflows worden beschreven. Als een bestandspatroon is opgegeven, hebben de uitvoerbestanden de namen die in het bestandspatroon worden beschreven.
   Naast de hierboven vermelde verplichte eigenschappen, ondersteunen Gecontroleerde mappen ook een aantal optionele eigenschappen. Zie [Gecontroleerde mapeigenschappen](/help/forms/using/watched-folder-in-aem-forms.md#main-pars-header-1)voor een volledige lijst en beschrijving van optionele eigenschappen.

## Central Migration Bridge (afgekeurd) gebruiken met een gecontroleerde map {#using-central-migration-bridge-deprecated-with-a-watched-folder}

U kunt een Gecontroleerde Omslag vormen om een werkschema, de dienst, of een manuscript in werking te stellen om de inputdossiers te verwerken. In de volgende sectie configureren we een gecontroleerde map om een ECMAScript te starten. ECMAScript zou OutputCentralService en sendToPrinter functionaliteit van SendToPrinterService gebruiken.
...
### Een ECMAScript maken {#create-an-ecmascript-1}

De ECMAScript zou de createPDF-API van de PDF-Generator gebruiken om Microsoft Word-documenten (.docx) naar PDF-documenten te converteren. Voer de volgende stappen uit om het script te maken:

1. Open CRXDE lijst in een browser venster. De URL is `https://[server]:[port]/crx/de`.

1. Navigeer naar /etc/workflow/scripts en maak een map met de naam **CMB**.

1. Maak in de map CMB een bestand met de naam cmb-mergeandprint-sample.ecma en voeg de volgende code toe aan het bestand:
...
1. Navigeer naar het knooppunt jcr:content van het bestand cmb-mergeandprint-sample.ecma en voeg een eigenschap jcr:title toe aan het knooppunt.

   >[!NOTE]
   >
   >Tijdens het maken van de workflow wordt de eigenschap title gebruikt om het ECMAScript te zoeken.

### Een workflow maken {#create-a-workflow-1}

1. Open de interface van de AEM-workflow in een browservenster. `https://[server]:[port]/workflow`
...
1. Klik met de rechtermuisknop op de processtap en selecteer **Bewerken**. Het venster Step Properties wordt weergegeven.
1. Selecteer op het tabblad Proces de ECMAScript. Bijvoorbeeld, cmb-mergeandprint-sample.ecma ECMAScript die in [Create een sectie van het Werkschema](/help/forms/using/watched-folder-in-aem-forms.md#p-create-a-workflow-p) wordt gecreeerd. Schakel de optie **Handler Geavanceerd** in en klik op **OK**.

### De gecontroleerde map configureren {#configure-the-watched-folder-1}

1. Open CRXDE lijst in een browser venster. `https://[server]:[port]/crx/de/`

1. Navigeer naar de map /etc/fd/watchfolder/config/ en maak een knooppunt met de naam CMBWatchedFolder van het type `nt:unstructured`.

1. Voeg de volgende eigenschappen toe aan het knooppunt:

   * **folderPath (String)**: Het pad van de map die met een bepaald tijdsinterval moet worden gescand. De map moet zich op een gedeelde locatie bevinden en alle servers moeten volledige toegang tot de server hebben.
   * **inputProcessorType (String)**: Het type proces dat moet worden gestart. Geef in deze zelfstudie een workflow op.
   * **inputProcessorId (String)**: Het gedrag van de eigenschap inputProcessorId is gebaseerd op de waarde die is opgegeven voor de eigenschap inputProcessorType. In dit voorbeeld is de waarde van de eigenschap inputProcessorType workflow. Voor de eigenschap inputProcessorId geeft u dus het volgende pad van de PDFG-workflow op:

      `/etc/workflow/models/cmb/jcr:content/model`

   * **outputFilePattern (String)**: Patroon van het uitvoerbestand. U kunt een map of bestandspatroon opgeven. Als een mappatroon is opgegeven, hebben de uitvoerbestanden de namen die in de workflows worden beschreven. Als een bestandspatroon is opgegeven, hebben de uitvoerbestanden de namen die in het bestandspatroon worden beschreven.

Naast de hierboven vermelde verplichte eigenschappen, ondersteunen Gecontroleerde mappen ook een aantal optionele eigenschappen. Zie [Gecontroleerde mapeigenschappen](/help/forms/using/watched-folder-in-aem-forms.md#main-pars-header-1)voor de volledige lijst en beschrijving van optionele eigenschappen.
