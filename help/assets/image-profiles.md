...
seo-description: Maak afbeeldingsprofielen met instellingen voor onscherp masker en kies voor slim uitsnijden of voor slim staal of beide om het profiel toe te passen op een map met afbeeldingselementen.
...
You use **Unsharp Mask** to fine-tune a sharpening filter effect on the final downsampled image. U kunt de intensiteit van het effect, de straal van het effect (gemeten in pixels) en een drempel voor het contrast die wordt genegeerd, instellen. Voor dit effect worden dezelfde opties gebruikt als voor het filter Onscherp masker van Adobe Photoshop.
...
Wanneer u slimme uitsnijdingen toevoegt aan een bestaand afbeeldingsprofiel, moet u de workflow [met](assets-workflow.md) DAM-updatemiddelen opnieuw activeren als u uitsnijdingen wilt genereren voor bestaande elementen in de gegevensopslagruimte.
...
1. Voer een van de volgende handelingen uit:
   ...
   * Ga naar **https://&lt;AEM server>/mnt/overlay/dam/gui/content/assets/foldersharewizard.html/content/dam** en pas het juiste profiel toe en tik op **Opslaan**.
