...
In dit artikel worden diverse rapporten beschreven over elementen in AEM Assets en hoe rapporten kunnen worden gegenereerd.
...
1. Kies op de pagina **[!UICONTROL Rapport]** maken het rapport dat u wilt maken en tik op **[!UICONTROL Volgende]**.

   ![choose_report](assets/choose_report.png)

   >[!NOTE]
   >
   >Voordat u een rapport **[!UICONTROL Asset Downloaded]** kunt genereren, moet u controleren of de service Asset Download is ingeschakeld. Open vanuit de webconsole (`https://[server name]:[port name]/system/console/configMgr`) de configuratie van de DAM-gebeurtenisrecorder **[!UICONTROL voor]** dagen van CQ en selecteer de optie **[!UICONTROL Gedownloade]** middelen (GEDOWNLOAD)in gebeurtenistypen als deze optie nog niet is geselecteerd.
...
1. Configureer rapportdetails zoals titel, beschrijving, miniatuur en mappad in de CRX-opslagplaats waar het rapport wordt opgeslagen. Standaard is het mappad */content/dam*. U kunt een ander pad opgeven.
   ...
   In het rapport **Koppeling delen** worden URL&#39;s weergegeven naar elementen die vanuit AEM-elementen worden gedeeld met externe gebruikers. Het bevat e-mailadressen van de gebruiker die de middelen heeft gedeeld, e-mailadressen van gebruikers met wie de middelen worden gedeeld, datum van delen en vervaldatum voor de koppeling. De kolommen kunnen niet worden aangepast.

   Het rapport **Koppeling delen** bevat geen opties voor submappen en uitvoeringen omdat het alleen de gedeelde URL&#39;s publiceert die onder */var/dam/share* worden weergegeven.
