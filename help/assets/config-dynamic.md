...
>[!NOTE]
>
>Zie Dynamische media [configureren - Scene7-modus](config-dms7.md)configureren als u van plan bent om Dynamic Media in Scene7 uit te voeren.
>
...
Ze worden beschreven in [Controle en onderhoud van uw AEMinstantie](/help/sites-deploying/monitoring-and-maintaining.md).
...
## Ondersteunde configuraties voor dynamische media {#supported-dynamic-media-configurations}
...
[Dynamische media](https://www.adobe.com/solutions/web-experience-management/dynamic-media.html) zijn standaard uitgeschakeld. Als u gebruik wilt maken van de dynamische mediafuncties, moet u dynamische media inschakelen in de **[!UICONTROL dynamicmedia]** uitvoermodus, zoals u bijvoorbeeld in de **[!UICONTROL publish]** uitvoermodus doet. Controleer voordat u de [technische vereisten](/help/sites-deploying/technical-requirements.md#requirements-for-aem-dynamic-media-add-on)inschakelt of deze zijn gecontroleerd.
...
Als u Dynamische media inschakelt, zijn de dynamische mediafuncties beschikbaar in de gebruikersinterface en ontvangt elk geüploade afbeeldingselement een uitvoering die wordt gebruikt voor snelle levering van dynamische afbeeldingsuitvoeringen. `cqdam.pyramid.tiff` Deze PTIFF&#39;s hebben belangrijke voordelen, zoals (1) de mogelijkheid om slechts één master afbeelding te beheren en op elk moment oneindige uitvoeringen te genereren zonder extra opslagruimte en (2) de mogelijkheid om interactieve visualisatie te gebruiken, zoals zoomen, pannen, centrifugeren, enzovoort.
...
1. Vraag `http://localhost:4502/is/image` en zorg ervoor de Server van het Beeld nu loopt.
   ...
   >[!NOTE]
   >
   >Raadpleeg de volgende logbestanden in de **[!UICONTROL crx-quickstart/logs/]** map voor het oplossen van problemen met Dynamic Media:
   >
   ...
   * s7access-&lt;yyyy>&lt;dd>.log - Het s7access logboek registreert elk verzoek aan Dynamische Media door `/is/image` en `/is/content`.
   Deze logboeken worden alleen gebruikt wanneer Dynamische media is ingeschakeld. Ze zijn niet opgenomen in het volledige **pakket** downloaden dat op de **[!UICONTROL system/console/status-Bundlelist]** pagina wordt gegenereerd. wanneer u Customer Support belt als u een probleem hebt met Dynamic Media, voeg dan beide logbestanden aan het probleem toe.
...
## (Optioneel) Dynamische mediavoorinstellingen en configuraties migreren van 6.3 naar 6.4 zonder downtime {#optional-migrating-dynamic-media-presets-and-configurations-from-to-zero-downtime}

Als u AEM Dynamische Media van 6.3 tot 6.4 bevordert - die nu de capaciteit voor nul onderbreking (ook genoemd &quot;Opt-in&quot;) plaatsingen omvat - moet u het volgende krullbevel in werking stellen om al uw voorinstellingen en configuraties van `/etc` aan `/conf` in CRXDE Lite te migreren.
...
De dynamische levering van het beeld van Media werkt door beeldactiva, met inbegrip van videoduimnagels, van Auteur AEM te publiceren en hen te herhalen aan Adobe op bestelling replicatieservice (de Replicatieservice URL). De activa worden dan geleverd door de dienst van de beeldlevering op bestelling (de Dienst URL van het Beeld).
...
1. Vouw op de **[!UICONTROL Edit User Settings For dynamic-media-replication]** pagina het **[!UICONTROL Add Private Key from KeyStore file]** gebied uit en voeg het volgende toe (zie de volgende afbeeldingen):
   ...
   * Voer in het **[!UICONTROL KeyStore File Password]** veld het wachtwoord voor het KeyStore-bestand in. Dit is _niet_ het KeyStore-wachtwoord dat u in stap 5 hebt gemaakt, maar de Adobe van het KeyStore-bestandswachtwoord staat in het welkomstbericht dat u tijdens de provisioning ontvangt. Neem contact op met de klantenservice van Adobe als u geen wachtwoord voor een KeyStore-bestand hebt ontvangen.
...
1. Tik op het **[!UICONTROL Settings]** tabblad en voer vervolgens het volgende in:
   ...
   * **[!UICONTROL Key Store Password]** - Dit is het KeyStore-wachtwoord dat u hebt gemaakt toen u hierop tikte **[!UICONTROL Create KeyStore]**. Adobe geeft dit wachtwoord niet op. Zie stap 5 van [Setting up Authentication](#setting-up-authentication).
...
>[!NOTE]
Voordat u Dynamic Media-Cloud Services instelt, moet u de publicatieinstantie hebben ingesteld. U moet replicatiereeks ook hebben alvorens Dynamische Cloud Services van Media te vormen.
...
1. Als u Dynamic Media voor video configureert,
   ...
   * Voer in het **[!UICONTROL Video Service URL]** veld de URL van de videoservice voor Dynamic Media Gateway in.
...
* De bevordering van AEM 6.3 aan AEM 6.4 of AEM 6.4.1, dan het vormen **[!UICONTROL Dynamic Media Configuration (Pre 6.3)]**, leidt nog tot een rapportreeks. Dit probleem is bekend en moet worden opgelost voor AEM 6.4.2.
...
1. Navigeer AEM naar de volgende locatie in **[!UICONTROL CRXDE Lite]** (hiervoor zijn beheerdersrechten vereist):
...
Bij niet-dynamische media-implementaties dupliceert u _alle_ elementen (zowel afbeeldingen als video) van de AEM auteursomgeving naar het AEM publicatieknooppunt. Deze workflow is nodig omdat de AEM ook de middelen levert.
...
Met de filters kunt u _uitsluiten_ dat elementen worden gerepliceerd naar het AEM publicatieknooppunt.
...
1. Navigeer in **[!UICONTROL Tools > General > CRXDE Lite]** het deelvenster `/conf/global/settings/dam/dm/imageserver/configuration/settings` met de standaard Adobe Color-profielen.
