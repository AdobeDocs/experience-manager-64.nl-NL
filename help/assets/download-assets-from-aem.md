...
1. Maak een map met een naamgevingsconventie die zich richt op de publicatie-runtime, namelijk `config.publish`:
   `/apps/<your-app-name>/config.publish`Om configuratieeigenschappen voor een looppaswijze te bepalen, zie Wijzen [van de](/help/sites-deploying/configure-runmodes.md#defining-configuration-properties-for-a-run-mode) Looppas voor meer informatie.
