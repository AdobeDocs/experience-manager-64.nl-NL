...
U kunt JSR 286-Volgzame portlets in AEM in werking stellen. Met de portletcomponent kunt u een portlet op de pagina insluiten. Zie [De AEM inhoudsportfolio beheren](#administering-the-aem-content-portlet).
...
Om AEM inhoud te vormen om in een portaal te verschijnen, zie [Installing, het Vormen, en het Gebruiken AEM in portlet](#installing-configuring-and-using-aem-in-a-portlet).
...
1. Ervan uitgaande dat de portalserver op host **localhost**, **port 8080 **wordt uitgevoerd en dat de AEM portlet webtoepassing wordt gemonteerd in de context **cqportlet**, is `http://localhost:8080/cqportlet/cqbridge/cqpcache?Path=$(path)` de url om de cache leeg te maken. Gebruik GET als methode.
...
1. Klik **Bewerken.**
1. Selecteer op het tabblad **Instellingen** het selectievakje **Enabled**, selecteer **Dispatcher Flush **als serialisatietype en voer een time-out voor opnieuw proberen in (bijvoorbeeld 60000).
...
1. In de OSGi configuratieconsole, door gebrek dat in http://localhost:4502/system/console/configMgr wordt gevestigd, selecteer **Handler van de Authentificatie van de SPELD CQ** van het drop-down menu.
...
De publicatieweergave heeft slechts één knop waarmee de werkbalk wordt verplaatst naar de weergave Beheren. De publicatieweergave wordt vertegenwoordigd door het bestand publish.html in [vorige bundel](/help/sites-deploying/configuring-osgi.md). In HTML, kunt u de volgende placeholders gebruiken, die door portlet met de respectieve inhoud worden vervangen wanneer teruggegeven:
...
De beheerweergave heeft vier knoppen: Bewerken, tabblad Websites, Vernieuwen en Vorige. De beheerde mening wordt vertegenwoordigd door het manage.html- dossier in [vorige bundel](/help/sites-deploying/configuring-osgi.md). In HTML, kunt u de volgende placeholders gebruiken, die door portlet met de respectieve inhoud worden vervangen wanneer teruggegeven:
...
Als u een aangepaste indeling wilt installeren, opent u de sectie **Bundles** van de OSGI-webconsole van de portlet en uploadt u de bundel.
...
1. Schakel het selectievakje **Titelbalk verbergen** in of uit, afhankelijk van het feit of u de titelbalk van de portlet wilt zien.
