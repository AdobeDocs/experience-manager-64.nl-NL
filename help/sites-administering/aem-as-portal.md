...
U kunt JSR 286-Volgzame portlets in AEM in werking stellen. Met de portletcomponent kunt u een portlet op de pagina insluiten. Zie [Het AEM inhoudsportfolio beheren](#administering-the-aem-content-portlet).
...
Om AEM inhoud te vormen om in een portaal te verschijnen, zie [Het installeren, het Vormen, en het Gebruiken van AEM in portlet](#installing-configuring-and-using-aem-in-a-portlet).
...
1. Ervan uitgaande dat de poortserver op de host wordt uitgevoerd **localhost**, **poort 8080 **en de AEM portlet webtoepassing wordt in de context gemonteerd **cqportlet**, is de URL om de cache leeg te maken `http://localhost:8080/cqportlet/cqbridge/cqpcache?Path=$(path)`. Gebruik GET als methode.
...
1. Klikken **Bewerken.**
1. In de **Instellingen** selecteert u de **Ingeschakeld** Schakel het selectievakje **Dispatcher Flush **in als het serialisatietype en voer een time-out voor opnieuw proberen in (bijvoorbeeld 60000).
...
1. In de OSGi configuratieconsole, door gebrek dat in http://localhost:4502/system/console/configMgr wordt gevestigd, selecteer **CQ PIN-verificatiehandler** in het keuzemenu.
...
De publicatieweergave heeft slechts één knop waarmee de werkbalk wordt verplaatst naar de weergave Beheren. De publicatieweergave wordt vertegenwoordigd door het bestand publish.html in [vorige bundel](/help/sites-deploying/configuring-osgi.md). In de HTML kunt u de volgende plaatsaanduidingen gebruiken, die door de portlet worden vervangen met de respectievelijke inhoud wanneer deze wordt weergegeven:
...
De beheerweergave heeft vier knoppen: Bewerken, tabblad Websites, Vernieuwen en Vorige. De beheerde mening wordt vertegenwoordigd door het manage.html- dossier in [vorige bundel](/help/sites-deploying/configuring-osgi.md). In de HTML kunt u de volgende plaatsaanduidingen gebruiken, die door de portlet worden vervangen met de respectievelijke inhoud wanneer deze wordt weergegeven:
...
1. Selecteer of ontruim **Titelbalk verbergen** Schakel het selectievakje in, afhankelijk van of u de titelbalk van de portlet wilt zien.
