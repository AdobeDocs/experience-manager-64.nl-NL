---
title: Opmerkingen bij de release AEM 6.4 Service Pack
seo-title: Opmerkingen bij de release AEM 6.4 Service Pack
description: De nota's van de versie specifiek voor Adobe Experience Manager 6.4 de Pakken van de Dienst.
seo-description: De nota's van de versie specifiek voor Adobe Experience Manager 6.4 de Pakken van de Dienst.
...
# Opmerkingen bij de release AEM 6.4 Service Pack {#aem-service-pack-release-notes}
...
AEM 6.4.8.0 is een belangrijke update die nieuwe eigenschappen omvat, zeer belangrijke klant gevraagde verhogingen en prestaties, stabiliteit, veiligheidsverbeteringen, die sinds de algemene beschikbaarheid van AEM 6.4 in **april 2018 worden vrijgegeven.**
...
* AEM Assets worden nu geconfigureerd met Brand Portal via Adobe I/O.
...
* Wanneer een URL van een pagina met AEM Sites een dubbelepunt of percentagesymbool bevat, reageert de onderliggende browser niet meer en geven CPU-cycli een punt weer (NPR-32368, NPR-31917).
* Wanneer een pagina AEM Sites wordt geopend voor bewerking en een component wordt gekopieerd, blijft de plakhandeling niet beschikbaar voor sommige plaatsaanduidingen (NPR-32328).
...
* Op de pagina met middelendetails van PDF-bestanden worden geen actieknoppen weergegeven in Experience Manager die worden uitgevoerd in de modus Dynamic Media Scene7 (NPR-32316).
...
* Het opnemen van PSD-elementen zorgt voor CPU-spike en een Experience Manager Author-instantie zonder respons (NPR-32165).
...
* De videocoderingen van Dynamic Media kunnen niet uploaden wanneer de Configuratie Scene7 Cloud in een privé omslag `/conf` in plaats van `/conf/global` (NPR-31779) wordt geplaatst.

* Afbeelding wordt niet weergegeven op de tijdlijn nadat annotaties zijn toegevoegd, op Experience Manager die wordt uitgevoerd in de Scene7-uitvoeringsmodus van Dynamic Media (NPR-31754).
...
* Als u het schema voor metagegevens niet publiceert in AEM Assets, wordt een foutbericht weergegeven, hoewel het schema op de achtergrond wordt verwijderd (CQ-4286871).
...
* Documentservices: Er wordt een uitzondering weergegeven bij het toepassen van de service Reader Extensions op een PDF-bestand (NPR-32639).
...
* Analytics geeft geen geschikte resultaten weer voor het verzenden of verlaten van formulieren op een sitepagina (NPR-31359).
...
AEM 6.4.7.0 is een belangrijke update die prestaties, stabiliteit, veiligheid en belangrijkste klantenmoeilijke situaties en verhogingen omvat die sinds de algemene beschikbaarheid van AEM 6.4 in **april 2018 worden vrijgegeven.**

Het is ook cumulatief, wat betekent dat 6.4.7.0 alle AEM 6.4 de dienstpakken vóór het vrijgeven omvat.

Enkele belangrijke hoogtepunten van AEM 6.4.7.0 zijn:
...
* De nieuwe kolom voor gecreeerde datum, die sorteerbaar is, is toegevoegd in **DAM lijstmening** en op activa onderzoeksresultaten in **Lijstmening** (NPR-31311).
* Asset sorting based on **Name** column is allowed in **List** view.
...
* Namen van middelen worden gewijzigd in kleine letters wanneer middelen van de ene map naar de andere worden verplaatst in AEM-systemen die in de runmode van Dynamic Media Scene 7 worden uitgevoerd (NPR-31631).
...
* De werkstroom van de Video van de Codering van Dynamic Media slaagt er niet in duimnagel voor de video te produceren die van Scene 7 aan Dynamic Media - de loopwijze van Scene 7 (NPR-31523) wordt gemigreerd.

* De interne Fout van de Server wordt waargenomen terwijl het gebruiken van filter aan onderzoek naar Reeksen, in AEM die op Dynamic Media - runmode Scene 7 (NPR-31388) loopt.
...
* De vervalwaarde (de tijd van het cliëntgeheime voorgeheugen aan levende) die voor Dynamic Media wordt gevormd Hybride wijze wordt niet herhaald aan het milieu van de levering van Dynamic Media (NPR-31126).

* Het uploaden van AEM-Dynamic Media - de runmode van Scene 7 naar Scene 7 duurt te lang om te voltooien (NPR-30926).

* Na het creëren van een pagina die Dynamic Media component terwijl het publiceren van het zelfde, van auteurinstantie die op Dynamic Media loopt - Scene 7 runmode, wordt de gebruiker ertoe aangezet om de configuratie dmscene7 (NPR-30880) te publiceren.

* De waarde van de parameter &quot;asset&quot; in de insluitcode van de viewer blijft ongewijzigd nadat de waarden in het veld &quot;Title after move&quot; en &quot;Name after move&quot; op Dynamic Media zijn gewijzigd - Scene 7 (NPR-30745).
...
* Verzamelingen worden verwijderd uit de weergave wanneer een filter wordt toegepast in AEM Assets die via Adobe Asset Link worden benaderd (CQ-4280534).
...
* Naam van element in AEM kan niet worden gesynchroniseerd met scène 7, wanneer een element met een dubbele naam al bestaat op scène 7 (CQ-4276763).
...
* De PPT-renderingworkflow genereert geen uitvoeringen van de geüploade PPT-bestanden, aangezien AEM 6.4 geen update naar com.adobe.granite.poi versie 2.0.28 (CQ-4279059) kan uitvoeren.
...
* Wanneer een pagina AEM Sites naar een andere locatie wordt verplaatst, worden de eigenschappen niet overeenkomstig bijgewerkt om de nieuwe locatie weer te geven (NPR-31265).
...
* Extra ondersteuning om een versie van een pagina in te stellen terwijl u deze verwijdert. Als versioning is uitgeschakeld voor de verwijderde pagina, kunnen AEM Sites dergelijke pagina&#39;s niet herstellen (NPR-30891).
...
* De integratie van Livefyre met AEM 6.4 verbetering geeft de Uitzondering van het Punt Null, wanneer de integratie gebruikend stop DITA voor synthetische middelen wordt gedaan. De integratie werkt echter wanneer componenten handmatig worden toegevoegd (FYR-11066).
...
* Informatie over lidgroepen/groepen is niet zichtbaar bij het maken van een nieuwe Site in een AEM-auteur-instantie (NPR-28840).
...
* De tijdelijke bestanden die door de Assembler Service worden gemaakt, worden niet automatisch verwijderd en moeten opnieuw worden opgestart (NPR-30846).
...
* Kan het Adobe PDF-instellingenbestand dat u hebt gedownload niet uploaden via de interface voor beheer (NPR-31273).

#### AEM 6.4.6.0 {#experience-manager-6460}

AEM 6.4.6.0 is een belangrijke update die prestaties, stabiliteit, veiligheid en belangrijkste klantenmoeilijke situaties en verhogingen omvat die sinds de algemene beschikbaarheid van AEM 6.4 in **april 2018 worden vrijgegeven.**

Het is ook cumulatief, wat betekent dat 6.4.6.0 alle AEM 6.4 de dienstpakken vóór het vrijgeven omvat.

Enkele belangrijke hoogtepunten van AEM 6.4.6.0 zijn:
...
* Koppeling voor het delen van elementen van een map met ruimte en `&` teken in de naam geeft voor bepaalde elementen lege grijze kaarten weer. NPR-29934: Hotfix voor CQ-4270187
...
* Het vormen van Dynamic Media Cloud Servicen op `DMHybrid` wijze resulteert in veelvoudige lege rapportreeksen die in Analytics zonder rapportreeks identiteitskaart worden gecreeerd die op AEM wordt opgeslagen resulterend in rapportsuite duplicatie. Hotfix voor CQ-4276855
...
* ContextHub-fout tijdens integratie met AEM en Campagne. NPR-30625: Hotfix voor CQ-4250790
...
* Unclosed ResourceResolver waarschuwing in ExperienceFragmentsReplicationListener die tot stabiliteitskwesties in tijd leidt, dwingt om instanties opnieuw te beginnen AEM. NPR-30416: Hotfix voor CQ-4257521
...
* Formulieren die na publicatie zijn gewijzigd, worden niet meer gepost bij het publiceren van de site. NPR-30411: Hotfix voor CQ-4236566

**Formulieren - Ondersteunende integratie**
...
* Als u een verbinding met een webservice toevoegt of bewerkt door webservices op te roepen vanuit AEM-formulieren Workbench, treedt er een fout op: ClassNotFoundException org.apache.axis.message.SOAPBodyElement. NPR-30116: Hotfix voor CQ-4273217
...
* Wanneer een laadtest wordt uitgevoerd op HTML naar PDF-service, mislukt dit met een fout en worden instellingen voor bestandstypen verwijderd van de AEM-formulierserver. NPR-30085: Hotfix voor CQ-4272631
...
AEM 6.4.5.0 is een belangrijke update die prestaties, stabiliteit, veiligheid en belangrijkste klantenmoeilijke situaties en verhogingen omvat die sinds de algemene beschikbaarheid van AEM 6.4 in **april 2018 worden vrijgegeven.**

Het is ook cumulatief, wat betekent dat 6.4.5.0 alle AEM 6.4 de dienstpakken vóór het vrijgeven omvat.

Enkele belangrijke hoogtepunten van AEM 6.4.5.0 zijn:
...
* Uitgeschakelde `AssetDownloadServlet` component OSGi door gebrek op AEM publiceert instanties. Zie Elementen [downloaden van AEM](/help/assets/download-assets-from-aem.md)voor meer informatie.
* Ondersteuning voor beheer van meerdere sites ingeschakeld voor middelen. Zie [Elementen hergebruiken met MSM voor Elementen](/help/assets/reuse-assets-using-msm.md)voor meer informatie.
...
* Nadat de elementtitel is toegevoegd aan de eigenschappenpagina, wordt de eigenschappenpagina opnieuw geopend wanneer de gebruiker de pagina probeert te sluiten. NPR-29628: Hotfix voor CQ-4264929
* Als u een groot aantal relaties op elementen maakt, treedt er een fout op. NPR-28779: Hotfix voor CQ-4250708
* De opname van activa is langzaam op Scene7 Connect looppas wijze. NPR-28658: Hotfix voor CQ-4263007
...
* De replicatie van AEM aan het Portaal van het Merk wordt vastgehouden voor lange periodes. NPR-28914: Hotfix voor CQ-4254932
...
* Als u de machtigingen voor het verwijderen van inhoud op de AEM-zijde uitschakelt, kan de gebruiker niet publiceren naar Brand Portal. Hotfix voor CQ-4270426
...
* De downloadfunctionaliteit gebruikt AEM Assets via een assetdownload-servlet, zodat anonieme gebruikers alle middelen kunnen downloaden. NPR-27315, hotfix voor CQ-4254732
...
* AEM Community-rapporten worden niet weergegeven in de auteur-instantie van AEM. Hotfix voor CQ-4266294
...
* Meerdere opgeslagen en gereflecteerde XSS (Cross-site scripting) in CRXDE Lite. NPR-29634: Hotfix voor GRANITE-26049
* De aanmeldingsfunctionaliteit voor het Delen van pakketten gebruikt GET verzoek in plaats van POST-verzoek, waardoor het wachtwoord onder het netwerktabblad zichtbaar wordt. NPR-29631: Hotfix voor GRANITE-26048
...
* Het veld Wachtwoord voor graniet wordt niet ingevuld na versleuteling. NPR-29245: Hotfix voor GRANITE-24908

**Integratie**

* Er wordt een uitzonderingsbericht weergegeven wanneer u de AEM-startconfiguratie probeert te bewerken en op te slaan. NPR-29086: Hotfix voor CQ-4266153
...
* Publiceer middelen van de omslag van AEM Author /content/dam/mac aan het Portaal van het Merk werkt niet. NPR-29820: Hotfix voor CQ-4271118
...
* CSRF-beschermingskader werkt niet met AEM-stichtingsformulieren. NPR-28612: Hotfix voor GRANITE-22231
...
De belangrijkste kenmerken voor AEM 6.4.5.0-formulieren zijn:
...
* Prefill de dienst ontbreekt toe te schrijven aan lange turnaround tijd van de module van de Integratie van Gegevens van AEM Forms. NPR-28997: Hotfix voor CQ-4265988
...
* Webservice-component in AEM Forms Workbench kan geen webservice aanroepen. Hiervoor is tweewegs-SSL-verificatie vereist. NPR-29485: Hotfix voor CQ-4246794
* AEM Forms JEE-configuratiemanager werkt niet met meerdere NIC-kaarten. NPR-29236: Hotfix voor CQ-4268598
* AEM-opstartfout die afkomstig is van GemFire: java.lang.IllegalStateException: Er kan slechts één AdminDistributedSystem-verbinding tegelijk worden gemaakt. NPR-29524: Hotfix voor CQ-4266295
...
* Ondersteuning voor beheer van meerdere sites ingeschakeld voor middelen. Zie [Elementen hergebruiken met MSM voor Elementen](/help/assets/reuse-assets-using-msm.md)voor meer informatie. NPR-26450: Hotfix voor CQ-4259922
...
Lijst met inhoudspakketten die zijn opgenomen in AEM 6.4.5.0

[Bestand ophalen](assets/6.4.5.0_OSGI.txt)

#### AEM 6.4.4.0 {#experience-manager-6440}

AEM 6.4.4.0 is een belangrijke update die prestaties, stabiliteit, veiligheid en belangrijkste klantenmoeilijke situaties en verhogingen omvat die sinds de algemene beschikbaarheid van AEM 6.4 in **april 2018 worden vrijgegeven.**

Het is ook cumulatief, wat betekent dat 6.4.4.0 alle AEM 6.4 de dienstpakken vóór het vrijgeven omvat.

Enkele belangrijke hoogtepunten van AEM 6.4.4.0 zijn:
...
* Toevoeging van de eigenschap Eén tekenreeks zonder naamruimte\[\] leidt tot een onvolledige terugverwijzing van XMP. NPR-26805: Hotfix voor CQ-4254142
...
* AEM OOTB PDF-handler blijft vastzitten bij het verwerken van PDF met externe tekens. NPR-28778: Hotfix voor CQ-4254234
* Wanneer een CSV-bestand een waarde heeft die door komma&#39;s in één kolom wordt gescheiden, wordt de komma niet door de AEM CSV-editor verwijderd en als een aparte kolom behandeld. NPR-28801: Hotfix voor CQ-4261694
...
* Bureaubladhandelingen werken niet voor resultaten van Omnzoekopdracht op `aem/start.html` pagina. NPR-27242: Hotfix voor CQ-4248176
...
* Metagegevens worden niet in gedownload element opgeslagen tijdens de eerste poging als Dynamic Media op de instantie zijn ingeschakeld. NPR-28233: Hotfix voor CQ-4260759
...
* Kan geen component bewerken na installatie van AEM 6.4.3. NPR-28125: Hotfix voor CQ-4261216
...
* De steunHTTP API om gelijkaardige beelden van binnen AEM Assets te vinden. Hotfix voor CQ-4254091
...
* (Adobe Stock Integration) Het modaal foutbericht van de Server wordt weergegeven met een Oauth-fout in het bestand error.log. Hotfix voor CQ-4260406
...
* Toegelaten toetsenbordnavigatie voor AEM Communities Enablement componenten op Publish. NPR-27728: Hotfix voor CQ-4254028
...
* (Cloud Service Configs) Het selectievakje &#39;Overgenomen van&#39; dat op het hoofdniveau wordt weergegeven, moet worden verwijderd. NPR-27856:  Hotfix voor CQ-4259676
...
* Omleiden naar URL werkt niet voor adaptieve formulieren die zijn ingesloten op een pagina AEM Sites. NPR-27620: Hotfix voor CQ-4239287
...
* Krabbelhandtekening werkt niet voor AEM Forms App die worden uitgevoerd op Apple iOS 12.1. NPR-28497: Hotfix voor CQ-4261765
...
**Formulieren - Transactierapportage**
...
Extra ondersteuning voor het vervangen van het gebruik van de clientbibliotheek van handlebars door onderstrepingsteken in de wizard voor het controleren van het begin van Forms Manager en de wizard voor het verplaatsen van elementen. NPR-27643: Hotfix voor CQ-4246536.
Eén bundel blijft geïnstalleerd nadat het Forms Management-pakket op de vertakking release/640 is geïnstalleerd. Hotfix voor CQ-4265410Formulieren die met bijlagen worden verzonden, worden niet weergegeven in de workflow met de verzendactie &quot;Workflow AEM Forms aanroepen&quot; en portaalverzending ingeschakeld. Hotfix voor CQ-4263110
...
* Wanneer AEM Forms worden opgenomen in IBM WebSphere, mislukt het maken van een formuliergegevensmodel op basis van SOAP. NPR-27923: Hotfix voor CQ-4251134
...
* Verschillende verbeteringen in variatieverschillen tussen inhoudsfragmenten voor AEM 6.4.  NPR-27866: FP voor CQ-4248839
* Functie Ingeschakeld in **AEM Workflow-terugkoppelingsmarkering** voor OSGI-configuratie. De actie intrekken moet de werkstroominstantie beëindigen nadat de markering is ingesteld. NPR-26451: Hotfix voor CQ-4259090
...
Lijst met inhoudspakketten die zijn opgenomen in AEM 6.4.4.0

[Bestand ophalen](assets/osgi_package_6_440.txt)

#### AEM 6.4.3.0 {#experience-manager-6430}

AEM 6.4.3.0 is een belangrijke update die prestaties, stabiliteit, veiligheid en belangrijkste klantenmoeilijke situaties en verhogingen omvat sinds de algemene beschikbaarheid van AEM 6.4 in April 2018 wordt vrijgegeven.

Het is ook cumulatief, wat betekent dat 6.4.3.0 alle AEM 6.4 de dienstpakken vóór het vrijgeven omvat.

Enkele belangrijke hoogtepunten van AEM 6.4.3.0 zijn:
...
* Uitgebreide, op een deelvenster gebaseerde zoekopdracht naar elementen in AEM
...
* Adobe IMS-verificatie en ondersteuning voor Admin Consoles voor beheerde services toegevoegd.
...
* Wanneer de integratie van Dynamic Media wordt toegelaten, worden sommige meta-gegevenseigenschappen van Activa gewijzigd. De breedte, de hoogte en de locatiekenmerken worden niet weergegeven. NPR-27203; Hotfix voor CQ-4256258
* Dynamic Media gebruiken niet de gevormde volmacht voor sommige types van activa. NPR-25211; Hotfix voor CQ-4244871
...
* De wegbrowser voorspelt wordt gepubliceerd van AEM Assets naar het Portaal van het Merk zoals verwacht. Het gepubliceerde pad bij BP blijft echter /content/dam, die moet worden bijgewerkt. Hotfix voor CQ-4256240

**DAM - Creative Cloud**

* Het pictogram &#39;Adobe Middelen zoeken&#39; ontbreekt in de hoofdnavigatie van AEM. Hotfix voor CQ-4254343
...
* Dynamic Media gebruiken niet de gevormde volmacht voor sommige types van activa. NPR-10727; Hotfix voor CQ-4244871
...
* AEM-instantie die wordt uitgevoerd op standaard-Http-poort (80) kan imsmanifest.xml niet bereiken. NPR-27173: Hotfix voor CQ-4252211
* Opmerking verwijderen als antwoord voor QnA werkt niet als AEM Communities zijn ingesteld met DSRP. NPR-26247: Hotfix voor CQ-4252232
* Kan Adobe Storage niet aanroepen: 414 fout - Lange GET URI waargenomen wanneer gebruikers /content/community-components/en/search.html doorzoeken en auteurveld selecteren als een van de filters voor die zoekterm. NPR-26643: Hotfix voor CQ-4251303
...
* U kunt niet op een miniatuurafbeelding klikken in de map Community Site wanneer u deze weergeeft vanuit de beheerconsole op een AEM Author-exemplaar. NPR-26986: Hotfix voor CQ-4254036
...
* Wijzigingen in AEM-ervaringen die zijn gemaakt via de doelengine, worden niet gepubliceerd. NPR-24869: Hotfix voor CQ-4247832
...
De belangrijkste kenmerken voor AEM 6.4.3.0-formulieren zijn:

* Ingeschakelde ondersteuning voor een array/lijst met objecten met dynamische entiteitsvervanging.
* FIPS-compatibiliteit is ingeschakeld voor de uitgebreide workflow van Reader in Digital Signature, Reader Extensions, CryptoProvider en TrustStore.
...
* Als de taaknaam accolades bevat terwijl de workflow Formulieren wordt uitgevoerd, wordt een uitzondering weergegeven in de logboeken. Hotfix voor CQ-4256626
* Kan een zoeksjabloon niet openen in de werkruimte AEM Forms. Hotfix voor CQ-4255651

**Mobiele formulieren**

* Het afsluitingsbericht wordt niet weergegeven wanneer het datumveld wordt verlaten in AEM Forms die in Internet Explorer of Chrome als HTML zijn gerenderd. NPR-26483: Hotfix voor CQ-4239352
...
Lijst met inhoudspakketten die zijn opgenomen in AEM 6.4.3.0

[Bestand ophalen](assets/6.4.3.0_OSGI.txt)

#### AEM 6.4.2.0 {#experience-manager-6420}

AEM 6.4.2.0 is een belangrijke update die prestaties, stabiliteit, veiligheid en belangrijkste klantenmoeilijke situaties en verhogingen omvat die sinds de algemene beschikbaarheid van AEM 6.4 in **april 2018 worden vrijgegeven.**
Het is ook cumulatief, wat betekent dat 6.4.2.0 alle AEM 6.4 de dienstpakken vóór het vrijgeven omvat.

Enkele belangrijke hoogtepunten van AEM 6.4.2.0 zijn:
...
* De redacteur van de Pagina van Plaatsen voegt steun voor in-context het uitgeven en samenstelling met cliënt-zijcomponenten toe bouwt in React of Hoekig in combinatie met de Redacteur van het KUUROORD JS SDK <a href="../sites-developing/spa-walkthrough.md">van</a>AEM.
...
* Toegevoegde [integratie met Adobe Stock](/help/assets/aem-assets-adobe-stock.md) zodat de gebruikers Adobe Stock-middelen rechtstreeks vanuit de AEM-gebruikersinterface kunnen zoeken, voorvertonen, opslaan en in licentie geven. Zie Adobe Stock Assets [gebruiken met AEM Assets](https://helpx.adobe.com/experience-manager/kt/assets/stock-assets-feature-video-use.md)voor meer informatie.
...
* migrateAllAssets - Voeg ondersteuning toe voor batchverwerking en verbeter de AEM-methode die UUID toevoegt aan oude elementen. NPR-24861: Hotfix voor CQ-4242863 en CQ-4247874
...
* (Dynamic Media) Bijgewerkte DM-viewers naar versie 5.10.1 waarmee u kunt controleren op dubbele namen op de pagina met voorinstellingen voor afbeeldingen. Zie Dynamic Media Viewer bijwerken (5.10.1). NPR-24403: Hotfix voor CQ-4247554
...
* Voor genestelde componenten parsys, altijd eerste (met minst genestelde weg) bevredigend ontwerp wordt toegepast van veelvoudige beschikbare componenten. Zie [Ontwerppadresolutie](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/page-templates-static.html)voor meer informatie. NPR-24973: Hotfix voor CQ-4246276
...
* De Target-engine (mbox.js, at.js) gebruikt geen beheerde URL&#39;s en gebruikt URL&#39;s met dubbele punten die mogelijk mislukken bij bepaalde implementaties. NPR-25339: Hotfix voor CQ-4237854
...
* De status van de Manager van het pakket is onjuist na verbetering aan tak AEM 6.4. NPR-24551: Hotfix voor graniet-21750
...
* (AEM Publish) Het Bron van het scorebord ontbreekt met contextweg en toont leeg scherm. NPR-26155: Hotfix voor CQ-4251942
...
* Probleem verholpen waarbij bij het importeren van vertaalde projecten twee gelijktijdige POST-aanvragen worden ingediend, waardoor er een fout optreedt. NPR-24889: Hotfix voor CQ-4247638
...
* Samengevoegde Experience Fragments/Target Integration to AEM 6.4.2.0. Hotfix voor CQ-4248653
...
* Wanneer u een element met een percentageteken (%) in de elementnaam probeert te downloaden, resulteert dit in een leeg archief. Hotfix voor CQ-4250558
...
* (Upgrade van 6.3.2 naar 6.4) Voorinstellingen voor afbeeldingen vóór upgrade worden op de pagina Uitvoeringen weergegeven als &quot;Niet gepubliceerd&quot;, maar geven geen URL-knop als deze optie is geselecteerd. Hotfix voor CQ-4240406
...
* Als u de Analytics-voorinstelling van 6.3 naar 6.4 en hoger upgradet, wordt een nieuwe rapportsuite en voorinstelling voor Analytics gemaakt, waardoor de oudere rapportgegevens van de gebruiker verloren gaan. Hotfix voor CQ-4244529
...
* Als u Adobe Granite Workflow Core opnieuw start, worden de workflowstappen opnieuw uitgevoerd vanaf het midden, waardoor onnodige workflows worden gemaakt. NPR-25057: Hotfix voor graniet-22218
...
De belangrijkste kenmerken voor AEM 6.4.2.0-formulieren zijn:
...
* Toegevoegde steun om het vermogen [van](https://helpx.adobe.com/experience-manager/6-4/forms/using/transaction-reports-overview.html) transactierapporten bij plaatsingen op-gebouw te gebruiken.
...
* Kan de OOTB AEM-formuliercomponent niet gebruiken vanwege een inefficiënte query. NPR-25065: Hotfix voor CQ-4247256
...
* Handeling verzenden met gebruik van &#39;Forms Workflow&#39; Classic-ontwerpproblemen. Hotfix voor CQ-4236981
...
* PDF&#39;s optimaliseren door lettertypen selectief te verwijderen via één API-aanroep. NPR-23287

**Document Services**

* Uitvoerservice bevat niet de juiste codes voor toegankelijkheidsReader. NPR-24438, NPR-24439, NPR-24440, NPR-24441: Hotfix voor CQ-4243849, CQ-4243845, CQ-4243852, CQ-4243853
...
* Toegevoegde [integratie met Adobe Stock](/help/assets/aem-assets-adobe-stock.md) zodat gebruikers Adobe Stock-middelen rechtstreeks vanuit de AEM-gebruikersinterface kunnen zoeken, voorvertonen, opslaan en in licentie kunnen geven. Zie Adobe Stock Assets [gebruiken met AEM assets]voor meer informatie (https://helpx.adobe.com/experience-manager/kt/assets/using/stock-assets-feature-video-use.html). NPR-15779: Hotfix voor CQ-30857
* Toegevoegde ondersteuning voor dynamische voorwaardelijke metaschema. Zie [Metagegevens](/help/assets/cascading-metadata.md)trapsgewijs verwerken voor meer informatie. NPR-25189: Hotfix voor CQ-4237413
...
* Mogelijkheid om een metagegevensschema voor elementmappen in te stellen. Voor meer informatie, zie het Schema [van Meta-gegevens van de](/help/assets/folder-metadata-schema.md) Omslag en verwijs naar zijn Montages [van de](#configuration-settings-required-for-npr) Configuratie post AEM 6.4.2.0 installatie. NPR-21268: Hotfix voor CQ-4221574

**Sites**

* U kunt een inhoudsfragment bewerken zonder machtigingen voor verwijderen. Zie [Inhoudsfragmenten](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/customizing-content-fragments.html#AssetPermissions)aanpassen en uitbreiden voor meer informatie. NPR-25793: Hotfix voor CQ-4248750
* Er is een functie toegevoegd voor het annoteren van inhoudsfragmenten. For more information, see [Variations-Authoring Fragments](https://helpx.adobe.com/experience-manager/6-4/assets/using/content-fragments-variations.html#AnnotatingaContentFragment). NPR-25188: Hotfix voor CQ-4235336
* Versioning: Vergelijk inhoudsfragmenten naast elkaar. Zie [Inhoudsfragmenten](https://helpx.adobe.com/experience-manager/6-4/assets/using/content-fragments-managing.html#ComparingFragmentVersions)beheren voor meer informatie. NPR-25187: Hotfix voor CQ-4237412
* Verbeteringen in de afbeeldingseditor zijn terug te vinden op AEM 6.4.2.0. Zie [Afbeeldingseditor](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/image-editor.html)voor meer informatie. NPR-24467
...
Lijst met inhoudspakketten die zijn opgenomen in AEM 6.4.2.0

[Bestand ophalen](assets/6_4_2_0content-package-list.txt)

#### AEM 6.4.1.0 {#experience-manager-6410}

AEM 6.4.1.0 is een belangrijke update die prestaties, stabiliteit, veiligheid en belangrijkste klantenmoeilijke situaties en verhogingen omvat sinds de algemene beschikbaarheid van AEM 6.4 in April 2018 wordt vrijgegeven.
...
* XMP-metagegevensextractie door Scene7ListInfoProvider vergt veel bronnen. NPR-23754
...
* Met TargetContentImpl verloopt AEM traag tijdens langdurige query&#39;s. NPR-22359: Hotfix voor CQ-4236907
* De Target-engine (mbox.js, at.js) gebruikt geen beheerde URL&#39;s en gebruikt URL&#39;s met dubbele punten die mogelijk mislukken bij bepaalde implementaties. NPR-22434: Hotfix voor CQ-4237854
* In de Target-modus kunnen auteurs een component die van de blauwdruk is overgeërfd, wijzigen zonder de overerving te annuleren. NPR-22865: Hotfix voor CQ-4237907
...
* Het kenmerk Alt-tekst is verbroken in de component Dynamic Media. Hotfix voor CQ-4244081
* (DMS7) Het uitgeven van verre reeksen in AEM wordt niet beschreven in Scene7. Hotfix voor CQ-4243430
...
* (DM Hybrid) Met URL kopiëren voor gepubliceerde Video-elementen of Gemengde Mediasets wordt &quot;[[Object]object&quot;&quot; opgehaald in het dialoogvenster URL. Hotfix voor CQ-4245236, CQ-4245451
...
* (DMS7) Cloud Configuration: Onbekwaam om nieuwe inhoud met Scene7 na het bijwerken aan SP1 te synchroniseren. Hotfix voor CQ-4244437
...
* (DMS7) Upload parameters for knockout background do not pass the ipsApiService.log and so, do not work. Hotfix voor CQ-4240686
...
* De de werkschemastap Scene7 leest slechts Scene7 in de naam en de beschrijving maar verduidelijkt niet de werkschemastap in het werkschema van de Update DAM. Hotfix voor CQ-4237865

**DAM - Slimme tags**

* Verbeterde slimme tags geïntroduceerd. NPR-21951

**Formulieren**

AEM Forms worden geleverd via invoegpakketten en andere patchinstallatieprogramma&#39;s die bij de release worden geleverd. Zie AEM Forms releases voor meer informatie.

De belangrijkste hooglichten voor AEM Forms zijn:

* AEM Forms introduceert de mogelijkheid [van](https://helpx.adobe.com/experience-manager/6-4/forms/using/transaction-reports-overview.html) transactierapporten om transacties zoals ingediende formulieren, verwerkte documenten en gerenderde documenten bij de implementatie van uw AEM Forms te volgen en te houden. Het biedt inzicht in productgebruik en helpt zakelijke gebruikers de volumes van digitale verwerking te begrijpen.
...
* Onder `ACTION TAKEN` Sectie in het lusje van het Type van Geschil, &quot;Extra documenten om mijn eis te steunen&quot;heeft extra gebied het Type van Bewijs wordt bewaard &quot;in het. Hotfix voor CQ-4239873
...
* AF&#39;s worden niet weergegeven met het tabblad Elementen van het zijpaneel, maar worden wel weergegeven met de bladeroptie voor het eigenschapsdialoogvenster van AEM-formuliercomponenten. Hotfix voor CQ-4236751
...
* Formulieren die worden verzonden op een pre-upgradeserver, zijn niet aanwezig op de geüpgrade server. Hotfix voor CQ-4241490
...
* De ingang voor bewaarde en voorgelegde vormen wordt bevolkt in crx/de ondanks wij hebben de configuratie van DB die in het Ontwerp van het Portaal van Formulieren en de Configuratie van de Verzending wordt gegeven. Hotfix voor CQ-4240726
...
* Datumveld overlapt in mobiele formulieren. Hotfix voor CQ-4242256
* De voorlegging van het formulier voor Mobiele Vormen moet als transactie worden geregistreerd gebruikend de Dienst van de Opname van de Transactie. Hotfix voor CQ-4246166
...
* Het delegeren van een taaktoewijzing werkt niet in Forms Integration in OSGI Workflows. NPR-23709: Hotfix voor CQ-4243700
...
* Target Area zodat deze zichtbaar blijft, zelfs nadat de inhoud is gesleept en neergezet. Hotfix voor CQ-4244534
...
* (IC Editor) De inhoud van een documentfragment dat in een IC wordt gebruikt, moet automatisch worden aangepast aan de breedte van het Target-gebied. Hotfix voor CQ-4244094
...
* De functie voor verbeterde slimme tags is toegevoegd. Zie [Verbeterde slimme tags](https://helpx.adobe.com/experience-manager/6-4/assets/using/enhanced-smart-tags.html)voor meer informatie. NPR-21951: Hotfix voor CQ-4234883
* Introductie van verwijzingen naar AEM Assets in InDesign. Zie Verwijzingen naar [AEM Assets in InDesign](/help/assets/managing-linked-subassets.md)voor meer informatie. NPR-23386

**Sites**

* (Paginaontwerp) Verbeteringen in de afbeeldingseditor. Zie [Afbeeldingseditor](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/image-editor.html)voor meer informatie. NPR-24267: Hotfix voor CQ-4245502
...
Lijst met inhoudspakketten die zijn opgenomen in AEM 6.4.1.0
...
1. Op de pagina Productinformatie (*/system/console/productinfo *) moet nu de bijgewerkte versie van de tekenreeks &quot;Adobe Experience Manager, versie 6.4.8.0&quot; onder Geïnstalleerde producten staan.
...
>[!NOTE]
>
>AEM 6.4.8.0 bevat een nieuwe versie van het compatibiliteitspakket [voor](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq640/compatpack/AEM-FORMS-6.4.7.0-COMPAT)AEM Forms. Als u een oudere versie van het compatibiliteitspakket voor AEM Forms gebruikt en een update naar AEM 6.4.8.0 uitvoert, installeert u de nieuwste versie van het compatibiliteitspakket voor AEM Forms na de installatie van het Forms Add-on-pakket.
...
Voor informatie over het installeren van het cumulatieve installatieprogramma voor AEM Forms JEE en configuratie na-plaatsing, zie [AEM Forms JEE Patch Installer 0015](https://helpx.adobe.com/aem-forms/quick-fixes/6-4/jee-patch-0015.html).
...
De Uber Jar voor AEM 6.4.8.0 is beschikbaar in de [openbare gegevensopslagplaats](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.4.8/)van Adobe.
...
Deze fouten vereisen geen actie omdat ze geen invloed hebben op uw AEM-instantie.
...
* Kan geen afbeeldingscomponent en pagina-eigenschappen bewerken nadat 6.4.3.0 is toegepast boven op 6.4.2.0. CQ-4260316 &amp; CQ-4260441Om dit probleem aan te pakken:
   * Ga naar Pakketbeheer
   * Pakket &quot;cq-ui-wcm-admin-content-1.0.1004.zip&quot; opnieuw installeren
   * Compileer alle JSP&#39;s opnieuw `(http://<AEM HOST>:<AEM PORT/system/console/slingjsp)` OF start de instantie opnieuw.
...
Lijst met inhoudspakketten die zijn opgenomen in AEM 6.4.8.0
...
* [Contact opnemen met de klantenondersteuning](https://daycare.day.com/)
