...
* Voor look, Voel en UX
   * De JS-sjabloon, CSS en [extend/override Javascript](client-customize.md#extending-javascript) wijzigen
* Om de informatie beschikbaar aan het Malplaatje JS of aan het eindpunt van GET te wijzigen
   * De [Sociale component](server-customize.md#socialcomponent-interface) uitbreiden
...
* Een nieuwe aangepaste bewerking toevoegen
   * Nieuwe [Sling Post Operation](server-customize.md#postoperation-class) maken
...
1. Geeft de bron (resourceType) door aan `SocialComponentFactoryManager`en ontvangt een SocialComponentFactory die een `SocialComponent`die de bron vertegenwoordigt kan selecteren.
...
Meer informatie over de verwerking van UGC die in [community content store](working-with-srp.md) wordt opgeslagen, raadpleegt u
