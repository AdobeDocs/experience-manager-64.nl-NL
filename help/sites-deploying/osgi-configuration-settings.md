...
* Als u wilt beschikken over de gebruikersinterface met aanraakfunctionaliteit als de standaardinterface, moet **Doelpad** naar het volgende verwijzen:
...
**CQ-foutopsporingsfilter van de dag (WCM)** Dit is handig bij het ontwikkelen omdat achtervoegsels zoals ?debug=layout kunnen worden gebruikt bij het openen van een pagina. Bijvoorbeeld, zal http://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout lay-outinformatie verstrekken die van belang voor de ontwikkelaar kan zijn.
...
* **URL om** gegevens te verzenden vormt URL die wordt gebruikt om paginatistieken te volgen (is essentieel als een trackerverzoek door de verzender gaat); De standaardwaarde is bijvoorbeeld  `http://localhost:4502/libs/wcm/stats/tracker`.
