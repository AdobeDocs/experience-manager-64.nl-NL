...
* **Naam van installatiemappen regexp** en **Maximale hiërarchiediepte van installatiemappen** - opgeven waar en tot welke diepte de opslagplaats naar bronnen zoekt die moeten worden geïnstalleerd. Wanneer een jokerteken wordt gebruikt (zoals in.&amp;ast;/install) alle geschikte overeenkomsten worden doorzocht, bijvoorbeeld `/libs/sling/install` en `/libs/cq/core/install`.
...
Er zijn [twee UI&#39;s](/help/sites-authoring/select-ui.md) beschikbaar in AEM:
