...
Er zijn twee systeemmodellen beschikbaar onder */libs/settings/mobileapps/model-types*. Als u de modeltypen van het systeem wilt overschrijven a *mobiele apps/modeltypen* de knoop zal onder de configuratieknoop moeten worden gecreeerd u wenst dat de opheffing voorkomt.

Als u bijvoorbeeld configuraties hebt gemaakt op */conf/myconf1* en */conf/myconf2* en wilt de modeltypen van het systeem overschrijven op *conf1* alleen maakt u een *mobiele apps/modeltypen* knooppunt onder de instellingen van *conf1*.

Als u gegevenstypen aan een model wilt toevoegen, moet het modeltype een onderliggende node hebben met de naam &#39;scaffolding&#39; van het type &#39;cq:Page&#39; en een resource type *wcm/steigers/onderdelen/steigers*.

De basispagina moet ook een *dataTypesConfig* eigenschap op het knooppunt PageContent die aangeeft welke gegevenstypemodellen van dit type mogen worden gebruikt.

>[!NOTE]
>
>A **Basisstructuur** is een pagina die de gegevenstypen definieert die kunnen worden bewerkt door een entiteit op basis van het model. Elk gegevenstype kan ook worden gevormd om te bepalen hoe het gebied in UI zal worden voorgesteld evenals hoe de gegevenswaarde zal worden voortgeduurd.
...
| fieldProperties | lijst met meerdere waarden van eigenschapcomponenten die worden gebruikt wanneer fieldPropResourceType is *mobileapps/caas/gui/components/models/editor/datatypes/field* |
...
Voor meer informatie over het Verdelen van Middel zie samenvoegen, [Het gebruiken van het Verzamelen van Middel in AEM](/help/sites-developing/sling-resource-merger.md).
...
Ga voor meer informatie over het maken en beheren van een model op het dashboard naar [Een model maken](/help/mobile/administer-mobile-apps.md) onder ontwerpsectie voor mobiele apps.
...
>[!NOTE]
>
>De *toegestane kinderen* en *toegestane ouders* eigenschappen volgen dezelfde regels als paginasjablonen. Zie voor meer informatie [Paginasjablonen](/help/sites-developing/page-templates-static.md).
>
>Zie *Modeltype* eigenschap, alle modellen moeten een supertype hebben *mobiele apps/cas/components/data/entity* maar kan een subtype hebben waardoor de levering van de inhoud kan worden aangepast. Als u ervoor zorgt dat alle modeltypen uniek zijn, kunnen clients van inhoudsservices ook een onderscheid maken tussen objecten in de gegevens.
...
>[!NOTE]
>
>Alle modellen zijn Malplaatjes, zodat volgen zij alle AEM regels van het Malplaatje. Dit maakt het gebruik van eigenschappen als *allowedParents* en *allowedChildren* eigenschappen. Deze zijn effectief bij het maken van nieuwe entiteiten op basis van een model. De sjabloonregels zorgen ervoor dat entiteiten alleen op bepaalde modellen kunnen worden gebaseerd, afhankelijk van hun hiërarchie.
>
>Ga voor meer informatie over het bewerken van een model via het dashboard naar [Een model maken](/help/mobile/administer-mobile-apps.md) onder ontwerpsectie voor mobiele apps.
...
**Paginamodel** Het model van Pagina&#39;s verstrekt een snelle methode om bestaande inhoud van Plaatsen voor levering door inhoudsdiensten opnieuw te gebruiken.
...
**Elementmodel** Het middelenmodel biedt een snelle methode voor het hergebruiken van bestaande inhoud van Elementen voor levering door inhoudsservices.

Het resourceType van entiteiten die op het model van Pagina&#39;s worden gebaseerd is: *mobileapps/caas/components/data/assets.*

Lijst met elementen: Lijst met paden van elementen. Elk element wordt toegevoegd als een onderliggende entiteitsknooppunt met een resourceType van *wcm/stichting/componenten/afbeelding*.

>[!NOTE]
>
>Ga voor meer informatie over het gebruik van deze sjablonen voor het maken van modellen op het dashboard naar [Een model maken](/help/mobile/administer-mobile-apps.md) onder ontwerpsectie voor mobiele apps.
