...
Het veld **Alternatieve tekst** is beschikbaar in het dialoogvenster **Afbeelding** in het tabblad **Metagegevens**:
...
AEM vereist dat het veld **Alternatieve tekst** standaard wordt ingevuld. Als de afbeelding zuiver decoratief is en alternatieve tekst onzinnig zou zijn, kan de optie **Afbeelding is decoratief** worden gecontroleerd.
...
* Illustratieve foto&#39;s: Dit zijn afbeeldingen van mensen, objecten of plaatsen. Denk na over de rol van de foto op de pagina. Een geschikt tekstequivalent zal waarschijnlijk *Foto zijn van [object]*, maar kan afhankelijk zijn van de omringende tekst.
...
* Grafieken en grafieken: Deze vertegenwoordigen meestal numerieke gegevens. U kunt dus een alternatief voor tekst bieden door een korte samenvatting op te nemen van de belangrijkste trends die in de grafiek of afbeelding worden weergegeven. Geef indien nodig ook een gedetailleerdere beschrijving in de tekst op met het veld **Beschrijving** op het tabblad Afbeeldingseigenschappen **Geavanceerd**. Bovendien kunt u de brongegevens elders op de pagina of site in tabelvorm opgeven.
   ...
   ![Voorbeeld van een grafiek. Hieronder ziet u de beste manier om een alternatief te bieden.](assets/chlimage_1.jpeg)
...
* heeft mogelijk geen toegang tot informatie zoals deze door een auteur wordt voorgesteld in een *standard* tweedimensionale, gekleurde webpaginalay-out met meerdere kolommen
...
AEM maakt het eenvoudig om webpagina&#39;s samen te stellen met behulp van de juiste HTML-elementen. Open de pagina-inhoud in de RTE (een tekstcomponent) en gebruik het menu **Paraformat** (alineasymbool) om het juiste structuurelement op te geven (bijvoorbeeld alinea, kop, enz.).
...
* **Lijsten gebruiken:** Met HTML kunt u drie verschillende typen lijsten opgeven:
   ...
   * Het element `<ol>` wordt gebruikt voor *genummerde lijsten*. Afzonderlijke lijstitems worden geïdentificeerd met behulp van het element `<li>`.
...
* **Tabellen** gebruiken:
   ...
   * a `<td>` element voor elke gegevenscel
   ...
   >[!NOTE]
   In de klassieke UI, zouden de lijsten met de **component moeten worden gerealiseerd**.
   ...
   * U kunt het beste standaardwaarden voor **Breedte**, **Hoogte**, **Rand**, **Celopvulling** en **Celafstand** verwijderen aangezien deze eigenschappen in een globaal opmaakmodel kunnen worden ingesteld.
   ...
   ![Dialoogvenster Tabeleigenschappen.](assets/chlimage_1-205.png)
...
* **Complexe gegevenstabellen:**
   ...
   >[!NOTE]
   In klassieke UI, zouden de lijsten met de **component moeten worden gerealiseerd**.
...
* Onjuist voorbeeld:
   ...
   * Tekst: Klik <u>hier</u> voor meer informatie over onze avondklassen voor het najaar van 2010.
   * Reden: de bestemming ervan wordt niet duidelijk en ondubbelzinnig aangegeven.

* Goed voorbeeld:

   * Tekst: <u>Gebeurtenisklassen voor herfst 2010</u> - details.
...
* Als een pagina in het Engels wordt geschreven, moet het `<html>`-element als volgt worden gelezen: `<html lang = “en-gb”>`
...
![Extra informatie (het woord is vereist) toevoegen aan schermlezers in het veld Titel.](assets/chlimage_1-210.png)
